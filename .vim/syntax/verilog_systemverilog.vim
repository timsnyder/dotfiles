...
if exists("sv_fold")
    syn region svFold matchgroup=verilogStatement   start="\<class\>" end="\<endclass\>" transparent fold keepend
else 
...
if exists("sv_fold")
    syn region svFold matchgroup=verilogStatement   start="\<\(rand\)\=sequence\>" end="\<endsequence\>" transparent fold keepend
else 
...
if exists("sv_fold")
    syn region svFold matchgroup=verilogStatement   start="\<interface\>" end="\<endinterface\>" transparent fold keepend
    syn region svFold matchgroup=verilogStatement   start="\<clocking\>" end="\<endclocking\>" transparent fold keepend
else 
    syn keyword verilogStatement   interface endinterface
    syn keyword verilogStatement   clocking endclocking
endif
syn keyword verilogStatement   modport cover covergroup coverpoint endgroup
if exists("sv_fold")
    syn region svPropertyDecl matchgroup=verilogStatement   start="\<property\>" end="\<endproperty\>" transparent fold keepend
    syn region svProgramDecl  matchgroup=verilogStatement   start="\<program\>"  end="\<endprogram\>" transparent fold keepend
else 
...
if version >= 508 || !exists("did_verilog_syn_inits")
   ...
   HiLink verilogAssertion       Include
    ...
    if exists("sv_include_perl")
	HiLink PrePerlTag verilogStatement
    endif
